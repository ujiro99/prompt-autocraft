...
1. キャレット位置までの文字列を取得
2. 特殊ケース: `//` 入力の場合、ピン留めプロンプトのみを表示
3. 通常ケース:
   - 単語境界を特定（正規表現: `(\S+)$`）
   - 検索語を抽出し、プロンプト名との大文字小文字を区別しない部分一致
   - マッチした候補を最大5件まで返却

#### ピン留めプロンプト専用トリガー (`//`)

ユーザーが `//` を入力した際、ピン留めされたプロンプトのみを表示する機能:

- **トリガー条件**: 
  - 入力が `//` で始まる（例: `//`）
  - または、空白文字の直後に `//` が続く（例: `text //`）
  - 誤トリガー防止: URL（`http://`）、トリプルスラッシュ（`///`）、単語内（`abc//`）では発動しない
- **動作**: 
  - 最小検索文字数制限をバイパス（通常3文字必要）
  - `isPinned: true` のプロンプトのみをフィルタリング
  - テキストフィルタリングなしで全てのピン留めプロンプトを表示
  - 最大候補数制限は通常通り適用（デフォルト5件）
- **用途**: 頻繁に使用するプロンプトへの高速アクセス
